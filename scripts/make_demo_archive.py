...
DEMO_ARCHIVES = [
    {
        ...
        "posts": [
            ...
            {
                # Г3 regression: пост с веткой комментариев для группового
                # ранжирования. Две participant_thread (marina_s и ivan_k),
                # плюс один «висячий» комментарий anna_p без reply_to —
                # для проверки что фильтрация работает корректно.
                "message_id": 900,
                "date": "2024-10-08T15:00:00",
                "username": "@philosophy_daily",
                "sender_type": "channel",
                "text": "Границы и ответственность. Граница — это не стена, за которой прячутся. Граница — это линия, на которой видно, где заканчиваюсь я и начинается другой. Без такой линии невозможно сказать «это моё, а это твоё» — ни про время, ни про чувства, ни про ответственность. Ответственность без границ выгорает: я отвечаю за то, что не моё. Границы без ответственности изолируют: я никому не должен, но и мне никто не должен. Здоровая граница держится только вместе со здоровой ответственностью.",
                "comments": [
                    # Ветка 1: marina_s ↔ author про «как сказать нет»
                    {"message_id": 901, "date": "2024-10-08T15:20:00", "username": "@marina_s",
                     "reply_to": 900,
                     "text": "А как сказать «нет» так, чтобы не обидеть? У меня каждый отказ сопровождается пятью извинениями и ощущением вины."},
                    {"message_id": 902, "date": "2024-10-08T15:35:00", "username": "@philosophy_daily",
                     "reply_to": 901,
                     "text": "Вина в отказе — это не про другого, это про вашу внутреннюю границу, которая пока ещё согласна с тем, что «отказывать нельзя». Скажите себе сначала: я имею право не соглашаться. Потом уже формулируйте «нет» для другого."},
                    {"message_id": 903, "date": "2024-10-08T15:50:00", "username": "@marina_s",
                     "reply_to": 902,
                     "text": "То есть разделить: внутри — разрешение себе, снаружи — формулировка. А если другой всё равно обижается?"},
                    {"message_id": 904, "date": "2024-10-08T16:05:00", "username": "@philosophy_daily",
                     "reply_to": 903,
                     "text": "Обида другого — это его реакция на столкновение с реальностью, где вы отдельный человек. Это не ваша ответственность. Ваша — сказать правду корректно. Не более."},

                    # Ветка 2: ivan_k ↔ author про «границы vs стены»
                    {"message_id": 905, "date": "2024-10-08T16:30:00", "username": "@ivan_k",
                     "reply_to": 900,
                     "text": "А где граница между «моя ответственность» и «контроль»? В команде я часто беру на себя чужое, потому что «иначе сделают плохо»."},
                    {"message_id": 906, "date": "2024-10-08T16:45:00", "username": "@philosophy_daily",
                     "reply_to": 905,
                     "text": "Контроль — это когда вы делаете за другого, чтобы получить нужный вам результат. Ответственность — когда вы обеспечиваете условия, при которых другой может сделать сам. Спросите себя: я сейчас делаю за него или создаю условия?"},
                    {"message_id": 907, "date": "2024-10-08T17:00:00", "username": "@ivan_k",
                     "reply_to": 906,
                     "text": "Понял. То есть контроль съедает чужую ответственность. Граница — это «я делаю свою часть и доверяю тебе твою»."},

                    # Висячий комментарий без reply_to (как старые демо-комментарии)
                    {"message_id": 908, "date": "2024-10-08T18:00:00", "username": "@anna_p",
                     "text": "Очень важная тема. У меня как раз сейчас ситуация с границами на работе."},
                ],
            },
        ],
    },
...
def build_db(db_path: Path, archive: dict) -> None:
    ...
    for post in archive["posts"]:
        ...
        # Комментарии. reply_to_msg_id — если в комментарии задано поле "reply_to",
        # иначе None (обратная совместимость со старыми комментариями).
        for c in post.get("comments", []):
            reply_to = c.get("reply_to")  # int | None
            cur.execute(
                "INSERT INTO messages (chat_id, message_id, date, username, text, "
                "post_id, is_comment, from_linked_group, sender_type, reply_to_msg_id, user_id) "
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (chat_id, c["message_id"], c["date"], c["username"], c["text"],
                 post["message_id"], 1, 1, "user", reply_to, 200600),
            )
