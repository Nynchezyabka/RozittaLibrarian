...
**Версия:** 5.2 (v1.7.3; ревизия документа под Claude 5: конфликт по таймауту download_media разрешён (#12), добавлен раздел «Формат патча», исправлена структура правил #20/#21, дедуп таблицы рефакторинга)
...
9. **Посты + комментарии** (канал + linked группа; ⚠️ BF-2 открыт — связывание пересланных постов с комментариями через `forward_from_message_id`)
...
### 12. Таймаут на download_media — зависит от размера файла

Файлы **>50 МБ** скачиваются БЕЗ таймаута: 120 секунд на большое видео через
медленное соединение не хватает — загрузка обрывается на середине.
Для остальных файлов — `wait_for(..., timeout=120)`.

```python
# ✅ ПРАВИЛЬНО:
LARGE_FILE_THRESHOLD = 50 * 1024 * 1024  # 50 МБ

async with self._sem:
    try:
        coro = message.download_media(file=target_path)
        size = (message.file.size if message.file else 0) or 0
        if size > LARGE_FILE_THRESHOLD:
            result = await coro                                   # без таймаута
        else:
...
# ❌ ЗАПРЕЩЕНО — безусловный wait_for(120) для всех файлов:
# большие видео стабильно обрываются по таймауту
```
...
ИНСТРУКЦИЯ_ДЛЯ_ИИ.md, CLAUDE.md и AGENTS.md (артефакты KB-пресета) — НЕ идентичные
копии: ИНСТРУКЦИЯ_ДЛЯ_ИИ.md — базовая инструкция, CLAUDE.md — базовая + добавка
...
5. Для больших методов допустим поиск по границам (см. ниже), но не «жадный» regex.
...
Перед написанием патча — прочитать актуальный файл (правило #16). Не угадывать
сигнатуры, имена методов и отступы: старый код в памяти ≠ текущий код на диске.

### Поиск метода по границам класса

1. Найти строку `class ClassName`
2. Найти следующий `class ` — нижняя граница
3. Внутри диапазона искать `def method_name`
4. Заменить от `def` до следующего `def` на том же уровне отступа

### Смена @staticmethod ↔ instance

Убрать декоратор недостаточно: добавить `self` в параметры и заменить внутренние
вызовы `ClassName.method()` на `self.method()`. Обратное преобразование — зеркально
(см. правило #20).

### Ручное исправление — валидный инструмент

Если патч ломает строковые литералы или не находит метод — точечная ручная правка
с номером строки и OLD → NEW блоком быстрее, чем патчи v12b, v12c...
...
| Строки с `\n` | Всегда `"\\n"` в патч-скрипте |
| Дублирующий код/аргументы | Сканирование файла + программное удаление |
| Мелкие правки (1-2 строки) | Ручное исправление — быстрее и надёжнее |
| Сигнатуры (self / @staticmethod) | Греп-проверка перед коммитом (правило #20) |
...
FILE = Path("features/export/generator.py")
